...
class ControllerShippingXshippingpro extends Controller {
	...
	public function index() {   
		...
		$this->load->model('xshippingpro/xshippingpro');
		...
		if($this->model_xshippingpro_xshippingpro->isDBBUPdateAvail()){
          $this->model_xshippingpro_xshippingpro->install();
        }
	   ...
	   $data['text_method_copy'] = $this->language->get('text_method_copy');
		...
		$data['method_data'] = $this->model_xshippingpro_xshippingpro->getData();
      ...
      public function quick_save(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->request->post['xshippingpro']) && isset($this->request->get['tab_id'])) {
				...
				$this->model_xshippingpro_xshippingpro->addData($save);
				$json['success']=1;
			}
			else {
	...
	public function delete() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'GET') && $this->validate()) {
			...
			if($this->request->get['tab_id']) {
				$this->model_xshippingpro_xshippingpro->deleteData($this->request->get['tab_id']);
				$json['success']=1;
			}
			else {
   ...
   public function export(){
		...
		$export['method_data'] = $this->model_xshippingpro_xshippingpro->getData();
   ...
   public function exportMethod($tab_id) {
		...
		$method_row = $this->model_xshippingpro_xshippingpro->getDataByTabId($tab_id);
				...
				if(is_array($import_data) && $import_data['xshippingpro_status']) {
					...
					if (isset($import_data['method_data']) && $import_data['method_data'] && is_array($import_data['method_data'])) {
						foreach($import_data['method_data'] as $single) {
							$this->model_xshippingpro_xshippingpro->addData($single);
						}
					}
					$success = true;
   ...
   private function validate() {
    ...
    public function uninstall(){        
       $this->load->model('xshippingpro/xshippingpro');
        $this->model_xshippingpro_xshippingpro->uninstall();
    }
