...
class ModelShippingSmsa extends Model {
	public function getQuote($address) {
		...
		foreach ($query->rows as $result) {
			if ($this->config->get('smsa_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$rates = explode(',', $this->config->get('smsa_' . $result['geo_zone_id'] . '_rate'));
				...
				if ((string)$cost != '') {
					$quote_data['smsa_' . $result['geo_zone_id']] = array(
						'code'         => 'smsa.smsa_' . $result['geo_zone_id'],
						'title'        => $result['name'] . '  (' . $this->language->get('text_weight') . ' ' . $this->weight->format($weight, $this->config->get('config_weight_class_id')) . ')',
						'cost'         => $cost,
						'tax_class_id' => $this->config->get('smsa_tax_class_id'),
