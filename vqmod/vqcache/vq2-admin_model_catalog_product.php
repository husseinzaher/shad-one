...
class ModelCatalogProduct extends Model {
	...
	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option` po INNER JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) INNER JOIN `" . DB_PREFIX . "option_description` od ON (po.option_id = od.option_id) WHERE po.product_id = '" . (int)$product_id . "' AND (o.type = 'select' OR o.type = 'radio' OR o.type = 'image' OR o.type = 'list') AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	return $query->rows;
}
public function getSwapOptionValues($product_id, $option_id) {
	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option_value` pov INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (pov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.option_id = '" . (int)$option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	return $query->rows;
}
public function getImageSwap($product_id) {
	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "image_swap` sw INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (sw.option_value_id = ovd.option_value_id) WHERE sw.product_id = '" . (int)$product_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY sw.sort_order, sw.id ASC");
	return $query->rows;
}
public function addProduct($data) {
	...
	if ($swap_list != '') {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "image_swap` WHERE `product_id` = '" . (int)$product_id . "' AND `id` NOT IN ($swap_list)");
	}
	else {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "image_swap` WHERE `product_id` = '" . (int)$product_id . "'");
	}
	foreach ($data['image_swap'] as $key => $swap) {
		if ($swap['id'] != '' && $swap['image'] != '') {
			$swap_id = $swap['id'];
			$this->db->query("UPDATE `" . DB_PREFIX . "image_swap` SET `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$swap['option_id'] . "', `option_value_id` = '" . (int)$swap['option_value_id'] . "', `image` = '" . $swap['image'] . "', `sort_order` = '" . (int)$swap['sort_order'] . "' WHERE `id` = '$swap_id' LIMIT 1");
		}
		elseif ($swap['image'] != '') {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "image_swap` SET `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$swap['option_id'] . "', `option_value_id` = '" . (int)$swap['option_value_id'] . "', `image` = '" . $swap['image'] . "', `sort_order` = '" . (int)$swap['sort_order'] . "'");
		}
	}
}
else {
...
if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image' || $product_option['type'] == 'list') {
