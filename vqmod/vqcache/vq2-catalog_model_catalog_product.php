...
public function checkSwap($product_id) {
	$query = $this->db->query("SELECT `option_id` FROM `" . DB_PREFIX . "image_swap` WHERE `product_id` = " . (int)$product_id . " LIMIT 1");
	if ($query->num_rows) {
...
public function getSwapBullets($product_id) {
	$query = $this->db->query("SELECT swap.option_value_id, swap.thumb, swap.image, swap.name FROM (SELECT sw.option_value_id, sw.image AS thumb, ov.image, ovd.name FROM `" . DB_PREFIX . "image_swap` sw INNER JOIN `" . DB_PREFIX . "option_value` ov ON (sw.option_value_id = ov.option_value_id) INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (sw.option_value_id = ovd.option_value_id) WHERE sw.product_id = '" . (int)$product_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY sw.sort_order, sw.id ASC) AS swap GROUP BY swap.option_value_id");
	return $query->rows;
}
public function getSwapThumbs($product_id) {
	$query = $this->db->query("SELECT swap.option_value_id, swap.name, swap.image FROM (SELECT sw.option_value_id, ovd.name, sw.image FROM `" . DB_PREFIX . "image_swap` sw INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (sw.option_value_id = ovd.option_value_id) WHERE sw.product_id = '" . (int)$product_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY sw.sort_order, sw.id ASC) AS swap GROUP BY swap.option_value_id");
	return $query->rows;
}
public function getSwapImages($product_id, $product_option_value_id = 0) {
	if ($product_option_value_id == 0) {
		$query = $this->db->query("SELECT `option_value_id` FROM `" . DB_PREFIX . "image_swap` WHERE `product_id` = '" . (int)$product_id . "' LIMIT 1");
		$option_value_id = $query->row['option_value_id'];
	}
	else {
		$query = $this->db->query("SELECT `option_value_id` FROM `" . DB_PREFIX . "product_option_value` WHERE `product_id` = '" . (int)$product_id . "' AND `product_option_value_id` = '" . (int)$product_option_value_id . "' LIMIT 1");
		$option_value_id = $query->row['option_value_id'];
	}
	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "image_swap` sw INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (sw.option_value_id = ovd.option_value_id) WHERE sw.product_id = '" . (int)$product_id . "' AND sw.option_value_id = '" . (int)$option_value_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY sw.sort_order, sw.id ASC");
	return $query->rows;
}
