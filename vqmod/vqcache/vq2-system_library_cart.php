...
class Cart {
				...
				if ($product_query->num_rows) {
					...
					foreach ($options as $product_option_id => $value) {
						...
						if ($option_query->num_rows) {
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image' || $option_query->row['type'] == 'list') {
	...
	$otp_swap_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "image_swap` WHERE `id` = '" . (int)$swap_id . "'");
...
$this->data[$key] = array(
	...
	public function add($product_id, $qty = 1, $option = array(), $recurring_id = 0, $swap_id = 0) {
		$this->data = array();
